...
class SyncUiState {
  const SyncUiState({this.running = false, this.last, this.at});
...
class SyncNotifier extends Notifier<SyncUiState> {
  ...
  /// Rapatrie le referentiel depuis le serveur.
  ///
  ...
  Future<SyncOutcome> refresh() async {
    ...
    state = SyncUiState(running: true, last: state.last, at: state.at);

    final outcome = await ref.read(syncRepositoryProvider).pullRooms();
    state = SyncUiState(last: outcome, at: DateTime.now());
    return outcome;
  }
}
